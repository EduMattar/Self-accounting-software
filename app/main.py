...
except ModuleNotFoundError as exc:  # pragma: no cover - surface helpful guidance
    if exc.name in {"tkinter", "_tkinter"}:
        raise ModuleNotFoundError(
            "Tkinter is required to run the desktop app, but it is not available in this "
            "Python installation. On macOS, install Python from python.org or, if you use "
            "Homebrew, run: brew install python-tk@3.13."
        ) from exc
    raise
...
try:  # pragma: no cover - import guard for script execution
    ...
    from app.accounts import AccountCatalog
    from app.journal import JournalBuilder
    from app.ledger import UniversalLedger, compute_unique_hash
    from app.mappings import PayeeMappingManager
    from app.models import CategorizedTransaction, TransactionRecord
